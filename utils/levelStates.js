const MENU = 0;
const GAME_OVER = 99;

// GAME_OVER -> LEVEL_X_START

const GAME_CLEARED = 399;

// LEVEL_X_TEXT : story dialogues
// LEVEL_X_START : start of level, ship reset, laser array clear, asteroid array reset
// LEVEL_X_GAME : gameplay, update, render, collision, timer, etc.
// LEVEL_X_END : end of level, story dialogues

const LEVEL_1_TEXT = 100;
const LEVEL_1_GAME = 110;
const LEVEL_1_END = 199;

const LEVEL_2_TEXT = 200;
const LEVEL_2_GAME = 210;
const LEVEL_2_END = 299;

const LEVEL_3_TEXT = 300;
const LEVEL_3_GAME = 310;



